...
def test_draw_winners_untrusted_ip(client, raffle, get_ticket):
    ...
    for n in range(raffle['data'][0]['total_tickets']):    
        get_ticket(raffle['data'][0]['raffle_id'])
             
    resp = client.post(f"/raffles/{raffle['data'][0]['raffle_id']}/winners/",REMOTE_ADDR='1.1.1.1')  
...
def test_draw_winners_tickets_remaining(client, raffle, manager_ip, get_ticket):
    ...
    resp = client.post(f"/raffles/{raffle['data'][0]['raffle_id']}/winners/", REMOTE_ADDR=manager_ip)
...
def test_draw_winners(client, raffle, manager_ip, get_ticket):
    ...
    for num in range(raffle['data'][0]['total_tickets']):    
        get_ticket(raffle['data'][0]['raffle_id'])

    resp = client.post(f"/raffles/{raffle['data'][0]['raffle_id']}/winners/", REMOTE_ADDR=manager_ip)
 
    assert resp.status_code == 201, unexpected_response_error(resp)
    wins = resp.json()['winners']
    ...
    assert  max(winning_numbers) <=raffle['data'][0]['total_tickets']
    assert min(winning_numbers) >= 1
    losing_numbers = set(range(1, raffle['data'][0]['total_tickets'] + 1)) - winning_numbers
    
    import pdb 
    pdb.set_trace()

    assert len(losing_numbers) >= 0
    assert len(losing_numbers) + len(winning_numbers) == raffle['data'][0]['total_tickets']
...
def test_draw_winners_already_drawn(client, raffle, manager_ip, get_ticket):
    ...
    resp1 = client.post(f"/raffles/{raffle['data'][0]['raffle_id']}/winners/", REMOTE_ADDR=manager_ip)
    assert resp1.status_code == 201, unexpected_response_error(resp1)

   
    resp2 = client.post(f"/raffles/{raffle['data'][0]['raffle_id']}/winners/", REMOTE_ADDR=manager_ip)
